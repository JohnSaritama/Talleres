...
def convertToCelsius(fahrenheit: Double): Option[Double] = {
  ...
  if (fahrenheit < -459.67)
    None
    //Si no se utiliza una variable imutable  en la cual se calcula el resultado
  else {
    // En esta parte realiza la formula planteada por el profesor
    val celsius = (fahrenheit - 32) * 5.0 / 9.0
    // se presenta primero la palabra some  como se pide y dentro la variable donde se almacena  el resultado
      Some(celsius)
    }
  }
